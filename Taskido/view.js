...
if (!taskFiles) {
    taskFiles = [...new Set(dv.pages().file.map((f) => f.tasks.filter((t) => !t.completed)).path)].sort();
} else {
...
function getMeta(tasks) {
    for (i = 0; i < tasks.length; i++) {
        ...
        while ((outerLink = /\[([^\]]+)\]\(([^)]+)\)/g.exec(tasks[i].text))) {
            tasks[i].text = tasks[i].text.replace(
                outerLink[0],
                "<a class='external-link outerLink' href='" + outerLink[2] + "'>" + outerLink[1] + "</a>",
            );
        }
        ...
        while ((innerLink = /\[\[([^\]]+)\]\]/g.exec(tasks[i].text))) {
            tasks[i].text = tasks[i].text.replace(
                innerLink[0],
                "<a class='internal-link innerLink' href='" + innerLink[1] + "'>" + innerLink[1] + "</a>",
            );
        }
    ...
    taskFiles.forEach(function (file) {
        ...
        var parentFolder =
            file.split("/")[file.split("/").length - 2] == null ?
                ""
            :   secondParentFolder + "📂&nbsp;" + file.split("/")[file.split("/").length - 2] + " / ";
...
function setEvents() {
    rootNode.querySelectorAll(".counter").forEach((cnt) =>
        cnt.addEventListener("click", () => {
            ...
            rootNode.classList.remove.apply(
                rootNode.classList,
                Array.from(rootNode.classList).filter((c) => c.endsWith("Filter") && !c.startsWith("today")),
            );
    ...
    rootNode.querySelector(".newTask").addEventListener("input", () => {
        ...
        if (newTask.includes("tomorrow ")) {
            input.value = newTask.replace("tomorrow", moment().add(1, "days").format("YYYY-MM-DD"));
        }
        if (newTask.includes("yesterday ")) {
            input.value = newTask.replace("yesterday", moment().subtract(1, "days").format("YYYY-MM-DD"));
        }
...
function getTimeline(tasks) {
    ...
    for (i = 0; i < timelineDates.length; i++) {
        ...
        var tasksFiltered = tasks
            .filter((t) => Object.values(t.happens).includes(timelineDates[i].toString()))
            .sort(eval(sort));
        ...
        tasksFiltered.forEach(function (item) {
            ...
            item.tags.forEach(function (tag) {
                ...
                if (hexColorMatch) {
                    var style =
                        "style='--tag-color:#" + hexColorMatch[1] + ";--tag-background:#" + hexColorMatch[1] + "1a'";
        ...
        rootNode.querySelector("span").appendChild(
            dv.el("div", date, {
                ...
                attr: { "data-year": year, "data-types": containedTypesPerDay.join(" ") },
            }),
        );
